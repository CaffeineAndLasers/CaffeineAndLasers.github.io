...
I am developing a novel wavefront sensor for use in Adaptive Optics assisted Free Space Optical Communications. The sensor I am developing uses liquid filled photonic crystal fibres (PCF).  The PCF splits the distorted beam into an array of weakly coupled waveguides, generating an interference pattern that encodes wavefront phase data. A neural network then reconstructs the wavefront from this pattern, bypassing the need for traditional lenslet arrays or short-wave infrared (SWIR) cameras. This design aims to reduce cost and complexity while improving scalability for satellite-to-ground FSOC links

![Scanning Electron Microscope photograph of a photonic crystal fiber](/Assets/PCF_SEM.png)
